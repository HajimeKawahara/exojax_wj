...
def model_c(nusd,y1,y1err,y2,y2err,onl=None):
    ...
    logg = numpyro.sample('logg', dist.TruncatedNormal(5.5,0.1,low=5.)) # Wang et al. (2022)
...
np.savez("./output_bn/H_mag.npz",[median_mu2,hpdi_mu2[0],hpdi_mu2[1]])
...
for i in range(len(mols_unique)):
    ...
    np.savez("./output_bn/"+mols_unique[i]+".npz",[median_mu3[i],hpdi_mu3[i][0],hpdi_mu3[i][1]])
